...
use clap::Parser;
use linfa::prelude::*;
use linfa_clustering::{KMeans, Dbscan, GaussianMixture};
...
impl ClusteringModel for KMeansModel {
    fn fit(&mut self, data: &Array2<f64>) -> Result<()> {
        let kmeans = KMeans::params(self.n_clusters)
            .max_iterations(100)
            .tolerance(1e-4)
            .fit(data)?;
...
impl ClusteringModel for DbscanModel {
    fn fit(&mut self, data: &Array2<f64>) -> Result<()> {
        let dbscan = Dbscan::params(self.eps, self.min_points)
            .fit(data)?;
        self.dbscan = Some(dbscan);
        Ok(())
    }
    
    fn predict(&self, data: &Array2<f64>) -> Result<Array1<usize>> {
        if let Some(ref dbscan) = self.dbscan {
            let predictions = dbscan.predict(data);
            Ok(predictions)
        } else {
            Err(anyhow::anyhow!("Model not fitted"))
        }
...
impl ClusteringBenchmark {
    ...
    fn evaluate_model(&self, data: &Array2<f64>) -> Result<HashMap<String, f64>> {
        if let Some(ref model) = self.model {
            ...
            metrics.insert("n_clusters".to_string(), unique_clusters.len() as f64);
        ...
        if mode == "training" {
            ...
            return Ok(BenchmarkResult {
                ...
                metadata: {
                    ...
                    meta.insert("n_clusters".to_string(), serde_json::Value::Number(serde_json::Number::from(quality_metrics.get("n_clusters").unwrap_or(&0.0)));
                    meta
                },
            });
