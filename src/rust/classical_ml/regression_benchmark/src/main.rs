...
use clap::Parser;
use linfa::prelude::*;
use linfa_linear::{LinearRegression, Ridge, Lasso, ElasticNet};
...
impl EnhancedRegressionBenchmark {
    ...
    fn load_boston_dataset(&self, n_samples: Option<usize>) -> Result<(Array2<f64>, Array1<f64>)> {
        ...
        let data = include_str!("../../../data/boston_housing.csv");
    ...
    fn load_california_dataset(&self, n_samples: Option<usize>) -> Result<(Array2<f64>, Array1<f64>)> {
        ...
        let data = include_str!("../../../data/california_housing.csv");
        let mut lines = data.lines();
        lines.next(); // Skip header
    ...
    fn create_model(&mut self, algorithm: &str, hyperparams: &HashMap<String, f64>) -> Result<()> {
        match algorithm {
            "linear" => {
                self.model = Some(Box::new(LinearRegression::new()));
            }
            "ridge" => {
                let alpha = hyperparams.get("alpha").unwrap_or(&1.0);
                self.model = Some(Box::new(Ridge::new().alpha(*alpha)));
            }
            "lasso" => {
                let alpha = hyperparams.get("alpha").unwrap_or(&1.0);
                self.model = Some(Box::new(Lasso::new().alpha(*alpha)));
            }
            "elastic_net" => {
                let alpha = hyperparams.get("alpha").unwrap_or(&1.0);
                let l1_ratio = hyperparams.get("l1_ratio").unwrap_or(&0.5);
                self.model = Some(Box::new(ElasticNet::new().alpha(*alpha).l1_ratio(*l1_ratio)));
            }
            _ => return Err(anyhow::anyhow!("Unknown algorithm: {}", algorithm)),
        }
        
        info!("Created {} model with hyperparameters: {:?}", algorithm, hyperparams);
        Ok(())
    }
    ...
    fn train_model(&mut self, x_train: &Array2<f64>, y_train: &Array1<f64>) -> Result<(f64, ResourceMetrics)> {
        ...
        self.model = Some(Box::new(fitted_model));
        
        let training_time = start_time.elapsed().as_secs_f64();
        let resource_metrics = self.resource_monitor.stop_monitoring();
        
        // Calculate model statistics
        let n_nonzero = if let Some(model) = &self.model {
            if let Some(coef) = model.coefficients() {
                coef.iter().filter(|&&x| x.abs() > 1e-10).count()
            } else {
                0
            }
        } else {
        ...
        let sparsity = if let Some(model) = &self.model {
            if let Some(coef) = model.coefficients() {
                1.0 - (n_nonzero as f64 / coef.len() as f64)
            } else {
                0.0
            }
        } else {
    ...
    fn calculate_r2_score(&self, y_true: &Array1<f64>, y_pred: &Array1<f64>) -> f64 {
        ...
        let ss_tot = (y_true - &y_true.mean().unwrap()).dot(&(y_true - &y_true.mean().unwrap()));
    ...
    fn run_inference_benchmark(&self, x_test: &Array2<f64>, batch_sizes: &[usize]) -> Result<HashMap<String, f64>> {
        ...
        for &batch_size in batch_sizes {
            ...
            // Warm-up runs
            for _ in 0..10 {
                let _ = model.predict(&x_test.slice(s![..batch_size, ..]));
            }
    ...
    fn get_hardware_config(&self) -> HardwareConfig {
        ...
        HardwareConfig {
            ...
            memory_gb,
            gpu_model: gpu_info.get("model").cloned(),
            gpu_memory_gb: gpu_info.get("memory_gb").cloned(),
            storage_type: "SSD".to_string(), // Default assumption
            storage_capacity_gb: 1000.0, // Default assumption
        }
    }

    fn get_gpu_info(&self) -> HashMap<String, Option<String>> {
...
impl EnhancedResourceMonitor {
    ...
    fn get_memory_usage(&self) -> usize {
        ...
        if let Ok(process) = sysinfo::Process::new(self.process_id, sysinfo::ProcessRefreshKind::new()) {
            process.memory()
        } else {
            0
        }
